...
- PCでは表を左1、右側を2の列幅比にし、右列にR選択、その下にPattern/Joker、その下に電卓を配置。SelectionとOddsの間隔を抑えつつ、電卓側を広く使える構成。作業領域をビューポート下端まで固定し、表パネルも同じ高さにする。表下の注意書きは削除し、最大12行のDDが画面内に収まる可変行余白のまま表文字を拡大。電卓は残りの高さを埋め、計算欄・結果・チップを拡大し、短い画面だけ電卓内で縦スクロールする。
