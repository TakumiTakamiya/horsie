...
- 作業ブランチは `main`、追跡先は `origin/main`。今回の開始時点は `ba6994a` で同期済み、マージ作業なし。
...
- 表・電卓・履歴テスト: `node --test scripts/calculator.test.cjs`（今回18件成功。見出し行削除、Selection→Odds→Probabilityの順序、確率列切り替え、説明削除と拡大表示も検証）
