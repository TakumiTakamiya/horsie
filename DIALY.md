...
- 右側に電卓を実装。三連単の結果選択（D2は4種類、DDは6種類）、共通の整数入力、単勝・2連単・三連単の倍率と計算結果3行、既存SVGによるチップ加算とクリア。入力欄を単勝行の横に置き、その下（2連単・三連単行の横）へチップ5種とクリアを2列×3行で配置する。計算欄下の常設説明2行は削除し、入力・倍率・結果の文字と行高を拡大。入力エラーだけ必要時に表示する。
