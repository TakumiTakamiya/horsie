...
- 作業ブランチは `main`、追跡先は `origin/main`。今回の開始時点は `2407b47` で同期済み、マージ作業なし。
...
- 表・電卓・履歴テスト: `node --test scripts/calculator.test.cjs`（今回17件成功。最大表行数12、注意書き削除、ビューポート高レイアウト、6列チップ配置も検証）
