...
- Tax Rate入力欄以外で数字キー（上段・テンキー）を押すと、フォーカス位置にかかわらず電卓入力へ追加し、入力欄へフォーカスを移す。初期値またはクリア後の `0` は最初の数字で置き換える。電卓入力欄とTax Rate入力欄では各入力欄の通常操作を優先する。
...
- 表・電卓・履歴テスト: `node --test scripts/calculator.test.cjs`（今回22件成功。券種別Tax Rate、階層型の丸め設定、0.1単位丸め、丸め単位別の表示桁、グローバル数字キー入力も検証）
