...
PC表示は左側にオッズ表、右側上段に操作、右側下段に電卓と数値表示設定を約2:1で配置します。900px以下では操作、表、電卓、設定の順に縦並びになります。
...
- Tax Rate入力欄以外では、フォーカス位置にかかわらず数字キーを押すと電卓へ入力されます。`Backspace` で末尾を1桁削除し、`C` キーで0へ戻ります。電卓入力欄は表示専用で、文字・記号・貼り付け入力やカーソル表示はありません。
