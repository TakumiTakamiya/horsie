...
スマートフォンでは電卓の下に配置し、ページを下へスクロールして操作します。オッズ表を表示しないため「的中確率を表示」は非表示にします。入力額とその他の表示設定はPC・スマートフォンで共通です。
...
- 全画面表示は、賭け額・倍率・払戻額の各数字が電卓上の位置から全画面上の位置へ拡大移動して開きます。移動中に画面をタップした場合は、移動完了後に続けてディーラー側へ180度回転します。移動後にタップした場合も同様に回転し、その次のタップで閉じます。端末の「視差効果を減らす」設定が有効な場合はアニメーションを省略します。
- 払戻ダイアログ内の文字は選択不可とし、スマートフォンで長押ししてもコピーや全選択のメニューを開きません。
