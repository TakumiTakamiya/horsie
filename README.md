...
PC表示は左側にオッズ表、右側上段に操作、右側下段に電卓と数値表示設定を約2:1で配置します。601〜900pxでは操作、表、電卓、設定の順に縦並びになります。600px以下ではスマートフォン専用表示となり、距離・統合したPattern/Joker・電卓・設定の順に並び、オッズ表は表示しません。
...
- 600px以下ではPatternとJokerを統合し、`DD`、`DDJ`、`DDJJ`、`D2`、`D2J`、`D2JJ`の6ボタンから一度に選択します。スマートフォン側の選択はPC側から独立しています。
